...
export class ServersService {
  ...
  async createServer(serverName: string, description: string) {
    try {
      const response = await axios.post('http://79.113.73.5.nip.io:8000/server/create', {
        name: serverName,
  ...
  async fetchServers() {
    try {
      const response = await axios.get('http://79.113.73.5.nip.io:8000/server/user/' + this.user.id);
      if (response.status === 200) {
  ...
  async joinServer(serverCode: string) {
    try {
      const response = await axios.post('http://79.113.73.5.nip.io:8000/server/join', {
  ...
  async getServer(serverId: number, userId: number) {
    try {
      const response = await axios.post('http://79.113.73.5.nip.io:8000/server', { server_id: serverId, user_id: userId });
      if (response.status === 200) {
        return response.data;
      } else {
  ...
  async fetchCategoriesAndRooms(serverId: number) {
    try {
      const response = await axios.get('http://79.113.73.5.nip.io:8000/server/' + serverId + '/categories');
      if (response.status === 200) {
        return response.data;
      } else {
