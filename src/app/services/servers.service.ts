...
export class ServersService {
  ...
  async createServer(serverName: string, description: string) {
    try {
      const response = await axios.post('https://192.168.1.134.nip.io/api/server/create', {
        name: serverName,
  ...
  async fetchServers() {
    try {
      const response = await axios.get('https://192.168.1.134.nip.io/api/server/user/' + this.user.id);
      if (response.status === 200) {
  ...
  async joinServer(serverCode: string) {
    try {
      const response = await axios.post('https://192.168.1.134.nip.io/api/server/join', {
  ...
  async getServer(serverId: number, userId: number) {
    try {
      const response = await axios.post('https://192.168.1.134.nip.io/api/server', { server_id: serverId, user_id: userId });
      if (response.status === 200) {
        return response.data;
      } else {
  ...
  async fetchCategoriesAndRooms(serverId: number) {
    try {
      const response = await axios.get('https://192.168.1.134.nip.io/api/server/' + serverId + '/categories');
      if (response.status === 200) {
        return response.data;
      } else {
