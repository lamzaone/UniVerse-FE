  ...
  private async checkToken() {
    ...
    if (token) {
      try {
        ...
        const response = await axios.post('http://79.113.73.5.nip.io:8000/auth/validate', { token });
        ...
        if (refreshToken) {
          try {
            ...
            const refreshResponse = await axios.post('http://79.113.73.5.nip.io:8000/auth/refresh', { token: refreshToken });
