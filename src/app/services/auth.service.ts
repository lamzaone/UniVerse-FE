  ...
  private async checkToken() {
    ...
    if (token) {
      try {
        ...
        const response = await axios.post('http://aetherial.cc:8000/auth/validate', { token });
        ...
        if (refreshToken) {
          try {
            ...
            const refreshResponse = await axios.post('http://aetherial.cc:8000/auth/refresh', { token: refreshToken });
