...
export class AuthService {
  ...
  private async checkToken() {
    ...
    if (token) {
      try {
        ...
        const response = await axios.post('https://192.168.1.134.nip.io/api/auth/validate', { token });
        ...
        if (refreshToken) {
          try {
            ...
            const refreshResponse = await axios.post('https://192.168.1.134.nip.io/api/auth/refresh', { token: refreshToken });
  ...
  private getGoogleOAuthUrl(): string {
    ...
    const redirectUri = encodeURIComponent('https://192.168.1.134.nip.io/auth/callback');
